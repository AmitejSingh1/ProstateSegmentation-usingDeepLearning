...
def save_results(image, mask, y_pred, save_image_path):
    ...
    success = cv2.imwrite(save_image_path, cat_images)
    if success:
        print(f"Image saved at {save_image_path}")
    else:
        print(f"Failed to save image at {save_image_path}")
...
if __name__ == "__main__":
    ...
    results_dir = "C:/Users/amite/OneDrive/Desktop/Prostate/results"
    create_dir(results_dir)

    """ Loading model """
    with CustomObjectScope({'iou': iou, 'dice_coef': dice_coef, 'dice_loss': dice_loss}):
        model = tf.keras.models.load_model("C:/Users/amite/OneDrive/Desktop/Prostate/files/model.keras")

    """ Load the dataset """
    test_x = sorted(glob('C:/new_data/val_png/images/*.png'))
    test_y = sorted(glob('C:/new_data/val_png/masks/*.png'))
